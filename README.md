...
`/reply` работает только для сообщений, полученных **после запуска клиента** —
в истории на диске нет packet id. Требует meshtastic с поддержкой `replyId`
(`pip install -U meshtastic`, если логгер вернёт ошибку).
