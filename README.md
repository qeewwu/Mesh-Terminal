...
- Команды: `/nodes`, `/who`, `/dm`, `/reply`, `/ch`, `/search`, `/updatenames`,
  `/clear`, `/help`; Tab-автодополнение команд, имён узлов и каналов
...
При старте показывает последние 50 сообщений из логов (настраивается через
...
| `/reply <текст>` | Ответ на последнее полученное сообщение — с настоящим `replyId`, получатели увидят цитату |
...
`Tab` дополняет команды, имена узлов после `/dm` и каналы после `/ch`.
`/reply` работает для сообщений, полученных после запуска клиента (история из
файлов не содержит packet id), и требует библиотеку meshtastic с поддержкой
`replyId` (`pip install -U meshtastic`, если логгер вернёт ошибку).
